...
def check_alertes(debarquement: Debarquement, details: list, db: Session):
    ...
    for detail in details:
        ...
        if espece.quota_mensuel_tonnes:
            ...
            total_mois = db.query(func.sum(DetailDebarquement.quantite_kg)).join(
                Debarquement
            ).filter(
            ...
            if total_mois_tonnes + (detail.quantite_kg / 1000) > espece.quota_mensuel_tonnes:
                debarquement.alerte_quota_depasse = True
                detail.alerte_quota = True
                alertes.append(f"Quota mensuel dépassé pour {espece.nom_commun_francais}")
...
def get_debarquements(
    ...
    for deb in debarquements:
        ...
        debarcadere = db.query(Debarcadere).filter(Debarcadere.id == deb.debarcadere_id).first()
        ...
        details = db.query(DetailDebarquement).filter(
            DetailDebarquement.debarquement_id == deb.id
        ).all()
        ...
        deb_dict['has_alertes'] = (
            deb.alerte_espece_protegee or
            deb.alerte_quota_depasse or
            deb.alerte_taille_illegale or
            deb.alerte_bateau_non_conforme
        )
...
@router.post("", response_model=DebarquementResponse, status_code=status.HTTP_201_CREATED)
def create_debarquement(debarquement_data: DebarquementCreate, db: Session = Depends(get_db)):
    """
    Créer un nouveau débarquement avec vérification d'alertes
    """
    # Vérifier que les entités existent
    debarcadere = db.query(Debarcadere).filter(Debarcadere.id == debarquement_data.debarcadere_id).first()
    if not debarcadere:
    ...
    pecheur = db.query(Pecheur).filter(Pecheur.id == debarquement_data.pecheur_principal_id).first()
    if not pecheur:
...
def get_stats_resume(
    ...
    # Alertes
    alertes_especes_proteges = query.filter(Debarquement.alerte_espece_protegee == True).count()
    ...
    alertes_bateaux = query.filter(Debarquement.alerte_bateau_non_conforme == True).count()
    ...
    return {
        ...
        "alertes": {
            ...
            "total": alertes_especes_proteges + alertes_quotas + alertes_tailles + alertes_bateaux
        }
    }
