...
class FonctionAgent(Base):
    """Fonctions Agents de surveillance"""

    __tablename__ = "fonctions_agents"

    id = Column(Integer, primary_key=True, index=True)
    libelle = Column(String(150), nullable=False)

    # agents = relationship("AgentSurveillance", backref="fonction")


class OrganismeAgent(Base):
    """Organisme Agents de surveillance"""

    __tablename__ = "organismes_agents"

    id = Column(Integer, primary_key=True, index=True)
    libelle = Column(String(150), nullable=False)
    abbreviation = Column(String(150), nullable=True)

    # agents = relationship("AgentSurveillance", backref="organisme")
...
class AgentSurveillance(Base):
    ...
    fonction_id = Column(Integer, ForeignKey("fonctions_agents.id"), nullable=True)
    organisme_id = Column(Integer, ForeignKey("organismes_agents.id"), nullable=True)
    contact_email = Column(String(100), nullable=True)
    contact_telephone = Column(String(20))

    fonction = relationship("FonctionAgent", backref="agents")
    organisme = relationship("OrganismeAgent", backref="agents")
