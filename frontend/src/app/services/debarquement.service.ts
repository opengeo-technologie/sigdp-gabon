...
export class DebarquementService {
  ...
  getDebarquements(filters?: {
    ...
    if (filters) {
      ...
      if (filters.debarcadere_id) params = params.set('debarcadere_id', filters.debarcadere_id.toString());
      if (filters.pecheur_id) params = params.set('pecheur_id', filters.pecheur_id.toString());
      if (filters.bateau_id) params = params.set('bateau_id', filters.bateau_id.toString());
      ...
      if (filters.avec_alertes !== undefined) params = params.set('avec_alertes', filters.avec_alertes.toString());
    }

    return this.http.get<Debarquement[]>(this.apiUrl, { params });
  }
  ...
  createDebarquement(debarquement: DebarquementCreate): Observable<Debarquement> {
    return this.http.post<Debarquement>(this.apiUrl, debarquement);
  }

  getDebarquementsAvecAlertes(skip?: number, limit?: number): Observable<Debarquement[]> {
    let params = new HttpParams();
    if (skip !== undefined) params = params.set('skip', skip.toString());
    if (limit !== undefined) params = params.set('limit', limit.toString());
    
    return this.http.get<Debarquement[]>(`${this.apiUrl}/alertes/actives`, { params });
  }
