...
@Component({
  selector: "app-bateaux-import",
  standalone: true,
  imports: [CommonModule],
  templateUrl: "./bateaux-import.component.html",
  styleUrl: "./bateaux-import.component.scss",
})
export class BateauxImportComponent implements OnInit {
  ...
  upload(): void {
    ...
    this.http
      .post<UploadResult>(
        `${environment.apiUrl}/api/bateaux/upload-excel`,
        formData,
      )
      .subscribe({
        next: (res) => {
          ...
          if (res.echoues === 0) {
            M.toast({
              html: `${res.inseres} bateaux importés avec succès.`,
              classes: "green",
            });
          } else {
            M.toast({
