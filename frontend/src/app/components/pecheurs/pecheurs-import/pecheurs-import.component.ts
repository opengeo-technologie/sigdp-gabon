...
@Component({
  selector: "app-pecheurs-import",
  standalone: true,
  imports: [CommonModule],
  templateUrl: "./pecheurs-import.component.html",
  styleUrl: "./pecheurs-import.component.scss",
})
export class PecheursImportComponent implements OnInit {
  ...
  upload(): void {
    ...
    this.http
      .post<UploadResult>(
        `${environment.apiUrl}/api/pecheurs/upload-excel`,
        formData,
      )
      .subscribe({
        next: (res) => {
          ...
          if (res.echoues === 0) {
            M.toast({
              html: `${res.inseres} pêcheurs importés avec succès.`,
              classes: "green",
            });
          } else {
            M.toast({
