...
export class MareyeurDetailsComponent implements OnInit {
  ...
  private pdfService = inject(MareyeurPdfService);
  ...
  imprimerAgrement(a: AgrementMareyage): void {
    ...
    this.pdfService.imprimerAgrement(this.mareyeur, a);
  }

  imprimerCarte(a: AgrementMareyage): void {
    if (!this.mareyeur) {
      return;
    }
    this.pdfService.imprimerCarte(this.mareyeur, a).catch(() =>
      M.toast({
